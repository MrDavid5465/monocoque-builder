...
const ClockTextNode: React.FC<ClockTextNodeProps> = ({ node, nodeAbsX, nodeAbsY, isSelected, registerCounterRotate, childEls }) => {
  ...
  const display = `${hh}:${mm}${node.showSeconds ? `:${ss}` : ''}${format === '12h' ? ` ${isPM ? 'PM' : 'AM'}` : ''}`;
